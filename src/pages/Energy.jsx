...
import { monthlyEnergy, monthKey } from '../analytics.js';
import { ChartCard, TrendChart, PieDonutChart } from '../components/AnalyticsCharts.jsx';
import EmptyState from '../components/EmptyState.jsx';
import { exportToCSV } from '../utils.js';
import {
  Zap, Plus, Trash2, Download, AlertCircle, Sun, Fuel, PlugZap, FolderOpen,
} from 'lucide-react';
...
export default function Energy() {
  ...
  const stats = useMemo(() => {
    ...
    const bySource = {};
    energy.forEach((e) => { bySource[e.source] = (bySource[e.source] || 0) + e.kwh; });
    return {
      today: Math.round(today.reduce((s, e) => s + e.kwh, 0)),
      month: Math.round(month.reduce((s, e) => s + e.kwh, 0)),
      total: Math.round(energy.reduce((s, e) => s + e.kwh, 0)),
      solarShare: (() => {
        const total = energy.reduce((s, e) => s + e.kwh, 0);
        const solar = energy.filter((e) => e.source === 'Solar Generation').reduce((s, e) => s + e.kwh, 0);
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          { icon: PlugZap, label: 'Today', value: `${stats.today.toLocaleString()} kWh`, cls: 'text-amber-400' },
          { icon: Zap, label: 'This Month', value: `${stats.month.toLocaleString()} kWh`, cls: 'text-cyan-400' },
          { icon: Fuel, label: 'Total Logged', value: `${stats.total.toLocaleString()} kWh`, cls: 'text-orange-400' },
              ...
              <tbody>
                {rows.map((r) => (
                  <tr key={r.id}>
                    ...
                    <td>
                      <span className="badge border" style={{ color: SOURCE_COLORS[r.source], borderColor: `${SOURCE_COLORS[r.source]}40`, backgroundColor: `${SOURCE_COLORS[r.source]}14` }}>
                        {r.source}
                      </span>
                    </td>
                    <td className="text-white font-semibold">{r.kwh.toLocaleString()}</td>
