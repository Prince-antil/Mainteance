...
import {
  ChartCard, TrendChart, PMCompletionChart, HorizontalBarChart,
  ParetoChart, PieDonutChart,
} from '../components/AnalyticsCharts.jsx';
import { useStore } from '../store.js';
import {
  computeKPIs, monthlyBreakdownTrend, monthlyPMCompletion, equipmentWiseBreakdown,
  paretoTop10, breakdownByDepartment, monthlyEnergy, healthDistribution,
  availabilityTrend, mttrTrend, mtbfTrend, buildInsights,
} from '../analytics.js';
...
export default function Dashboard() {
  ...
  const charts = useMemo(() => ({
    ...
    pmCompletion: monthlyPMCompletion(store.pms),
    equipment: equipmentWiseBreakdown(store.breakdowns, store.machines).slice(0, 8),
    pareto: paretoTop10(store.breakdowns, store.machines),
    dept: breakdownByDepartment(store.breakdowns, store.machines),
    ...
    <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div>
            ...
            <div className="flex flex-wrap items-center gap-2.5 mt-4">
              <StatusBadge
                ...
                label={kpi.breakdown > 0 ? `${kpi.breakdown} Machine${kpi.breakdown > 1 ? 's' : ''} Down` : 'Plant Systems Nominal'}
                pulse
              />
      ...
      <section aria-label="Key performance indicators">
        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4 lg:gap-5">
          ...
          <KPIStatCard icon={AlertOctagon} label="Breakdown Machines" value={kpi.breakdown} sub={kpi.breakdown ? 'Attention required' : 'No active failures'} tone={kpi.breakdown ? 'danger' : 'neutral'} pulse={kpi.breakdown > 0} />
          <KPIStatCard icon={ClipboardList} label="PM Due" value={kpi.pmPending} sub={`${kpi.pmOverdue} overdue`} tone={kpi.pmOverdue ? 'danger' : 'accent'} />
          <KPIStatCard icon={ClipboardCheck} label="Completed PM" value={kpi.pmCompleted} sub={`Compliance ${kpi.pmCompliance}%`} tone="success" />
          ...
          <KPIStatCard icon={ListChecks} label="Open Work Orders" value={kpi.openWorkOrders} sub={`${kpi.breakdownsThisMonth} breakdowns this month`} tone={kpi.openWorkOrders ? 'warning' : 'neutral'} />
        </div>
      </section>
        ...
        <div className="glass-card p-5">
          <div className="flex items-center gap-2.5 mb-4">
            ...
            <div>
              ...
              <p className="text-meta">Auto-generated from live breakdown, PM and health data</p>
            </div>
          </div>
          {insights.length === 0 ? (
            <p className="text-body py-2">Add machines and log breakdowns / PM tasks — recommendations will appear here automatically.</p>
          ) : (
      ...
      <section aria-label="Analytics charts" className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <ChartCard title="Monthly Breakdown Trend" subtitle="Breakdown count · last 6 months" empty={noBDs}>
          <TrendChart data={charts.bdTrend} dataKey="count" color="#EF4444" />
        </ChartCard>
        <ChartCard title="Monthly PM Completion" subtitle="Completed vs pending schedules" empty={noPMs}>
          <PMCompletionChart data={charts.pmCompletion} />
        </ChartCard>
        <ChartCard title="Equipment-wise Breakdowns" subtitle="Failure count per machine" empty={noBDs}>
          <HorizontalBarChart data={charts.equipment} color="#06B6D4" />
        </ChartCard>
        <ChartCard title="Top 10 Breakdown Machines" subtitle="Pareto — 80/20 failure analysis" empty={noBDs} height={280}>
          <ParetoChart data={charts.pareto} />
        </ChartCard>
        <ChartCard title="Breakdown by Department" subtitle="Failure distribution across plant" empty={noBDs}>
          <PieDonutChart data={charts.dept} />
        </ChartCard>
        <ChartCard title="Monthly Energy Consumption" subtitle="Total kWh · last 6 months" empty={noEnergy}>
