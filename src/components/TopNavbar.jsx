...
export default function TopNavbar() {
  ...
  useEffect(() => {
    api.getRecent().then(setUploadNotifs).catch(() => {});
  }, []);

  // Derived notification centre: PM overdue/upcoming, open breakdowns,
  // low health/availability (live) + new report uploads (server)
  const notifications = useMemo(() => {
    ...
    const uploads = uploadNotifs.slice(0, 6).map((n) => ({
      ...
      detail: `${n.uploader_name || 'System'} uploaded ${n.filename} · ${n.category_name}`,
  ...
  const localResults = useMemo(() => {
    ...
    store.breakdowns.forEach((b) => {
      const mName = store.machines.find((m) => m.id === b.machineId)?.name || '';
      if (b.complaintNo.toLowerCase().includes(q) || (b.problem || '').toLowerCase().includes(q) || mName.toLowerCase().includes(q)) {
        out.push({ kind: 'breakdown', title: `${b.complaintNo} — ${mName}`, sub: `Breakdown · ${b.status}`, to: '/breakdowns' });
      }
    });
    store.pms.forEach((p) => {
      const mName = store.machines.find((m) => m.id === p.machineId)?.name || '';
      if (mName.toLowerCase().includes(q) || p.frequency.toLowerCase().includes(q) || (p.engineer || '').toLowerCase().includes(q)) {
        out.push({ kind: 'pm', title: `${p.frequency} PM — ${mName}`, sub: `PM · due ${new Date(p.pmDate).toLocaleDateString('en-GB')}`, to: '/pm' });
      }
    });
            ...
            {notifOpen && (
              <div className="absolute right-0 top-full mt-2 w-[340px] glass-card !rounded-xl overflow-hidden shadow-2xl">
                ...
                {notifications.length === 0 ? (
                  <p className="px-4 py-4 text-slate-500 text-xs">All clear — no alerts. Overdue PMs, open breakdowns and low health warnings will surface here automatically.</p>
                ) : (
