...
import {
  ...
  addSparePart, removeSparePart, addMachinePhoto, removeMachinePhoto,
} from '../store.js';
...
export default function MachineProfile() {
  ...
  const handleFile = (f) => {
    ...
    reader.onload = () => {
      addMachineDoc(machine.id, { tab, filename: f.name, file_format: ext, file_url: reader.result, uploadedBy: userName }, userName);
      setUploading(false);
    };
  ...
  const handlePhoto = (f) => {
    ...
    const reader = new FileReader();
    reader.onload = () => addMachinePhoto(machine.id, { name: f.name, dataUrl: reader.result, addedBy: userName });
    reader.readAsDataURL(f);
  };
                    ...
                    return (
                      <li key={d.id} className="flex flex-col sm:flex-row sm:items-center gap-3 rounded-control border border-white/[0.06] bg-white/[0.02] px-4 py-3">
                        ...
                        <div className="flex items-center gap-2 flex-shrink-0">
                          <button onClick={() => openPreview(d)} className="btn-ghost inline-flex items-center gap-1.5 text-xs !py-1.5" aria-label={`Preview ${d.filename}`}>
                            <Eye size={12} aria-hidden="true" /> Preview
                          </button>
                          <a href={d.file_url} download={d.filename} className="btn-ghost inline-flex items-center gap-1.5 text-xs !py-1.5 text-cyan-400 hover:text-cyan-300" aria-label={`Download ${d.filename}`}>
                            <Download size={12} aria-hidden="true" /> Download
                          </a>
                          {isAdmin && (
                            <button onClick={() => removeMachineDoc(machine.id, d.id)} className="btn-ghost inline-flex items-center gap-1.5 text-xs !py-1.5 text-red-400 hover:text-red-300" aria-label={`Delete ${d.filename}`}>
                              <Trash2 size={12} aria-hidden="true" />
                            </button>
                          )}
                        </div>
